...
# Ändern Sie die Farbcodes, um die Board-GUI zu bearbeiten
class Color(Enum):
...
# mögliche Feldzustände
class FieldStatus(Enum):
    FREE = 0
    OCCUPIED = 1


# Spielerobjekt bestimmt, welche Farben verwendet werden, und verfolgt gewonnene Spiele
class Player(object):
...
class Game(object):
    ...
    # Erstelle Spielfelder und füge sie dem Board-Array hinzu
    ...
    # Erstellen Sie die Nachrichtenetiketten für das Ende des Spiels
    ...
    # Erstellen Sie eine Leinwand, um die Feldobjekte darin zu zeichnen
    ...
    # Binden Sie die Standardereignisse
    def bind_events(self):
        self.canvas.bind('<Button-1>', self.on_click)
        self.canvas.bind('<Motion>', self.on_motion)

    # Entfernen Sie die Standardbindungen. Mausklick setzt das Spiel jetzt zurück
    def after_win_events(self):
        self.canvas.bind('<Button-1>', self.reset_game)
        self.canvas.unbind('<Motion>')

    # Entfernen Sie die Endgame-Nachricht, setzen Sie das Board zurück und binden Sie die Standardereignisse
    ...
    # Was bei Mausbewegungen zu tun ist
    ...
    # Was mit einem Mausklick-Ereignis zu tun ist
    ...
    # Bestimmen Sie, ob das Spiel mit dem kürzlich beanspruchten Feld endete
    def is_end(self):
        winner = None
        ...
        for direction in directions:
            ...
            if len(streak) >= self.win_condition:
                ...
                return True
    # erkenne, ob das Brett voll ist, ohne dass jemand gewinnt
    ...
    # bestimmen, wie viele verbundene Felder in der ausgewählten Richtung gefunden werden können.
    # Beispiel: (1,0) wird entlang der x-Achse getestet
    ...
    # Anzeige der Game-Over-Nachricht
    def end_game(self, winner):
        self.after_win_events()
        if winner is not None:
            text = '{} Gewonnen'.format(winner.player_type.value)
            winner.add_point()
    ...
    # jedes Feld auf der Tafel zurücksetzen
    def reset_board(self):
        for column in self.board:
            for field in column:
                field.reset()

    # den Umriss jedes Feldes zurücksetzen (Hinweis: wird in der aktuellen Version nicht verwendet)
    def reset_soft(self):
        for column in self.board:
            for field in column:
                field.reset_outline()
...
class Field(object):
    ...
    # Setzen Sie das Feld auf Schweben
    ...
    # das Feld vollständig zurücksetzen
    def reset(self):
...
# Ermöglicht das Bearbeiten der Spieleinstellungen
class GameSettings(object):
    ...
    # Erstellen Sie das Einstellungsfenster mit seinen Widgets
    ...
    # Platzieren Sie ein beliebiges Tkinter-Widget an den ausgewählten Koordinaten
    def place(self, widget, x, y):
        widget.place(x=x, y=y, width=140, height=self.height)

    # Erstellen Sie ein Eingabefeld an der angegebenen Position (0 oder höher)
    ...
    # Erstellen Sie ein Label an der angegebenen Position (0 oder höher)
...
# Fügen Sie Tkinters Canvas eine Create-Circle-Methode hinzu
def _create_circle(self, x, y, r, **kwargs):
    return self.create_oval(x - r, y - r, x + r, y + r, **kwargs)
Canvas.create_circle = _create_circle


# Erstellt ein Tkinter-Fenster und zentriert es auf dem Bildschirm. Geben Sie das erstellte Fenster zurück
...
# Starte das Einstellungsfenster und warte auf Eingaben - starte das Spiel mit Standardwerten, wenn ValueErrors auftreten.
